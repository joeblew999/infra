...
Use the tasks folder to tracks work to do in agent/tasks.

When working with a person in planning mode, work up a task together with the person, updating the task until the person agres that its ready its ready for you to start work on it. 

Use a numbering system for each task. When a task is done put it in the task/done folder.  

In each task, checkbox as you work through the items, letting the person know. 
...
- Run `go run . tools dep list` to verify CLI integration
- Run `go run . api-check` to verify API compatibility
- Test installation with `go run . tools dep install <binary>`
...
- **`--cross-platform` flag**: Available for `go run . tools dep local install` command
...
- **Usage examples**:
  ...
  go run . tools dep local install garble --cross-platform
  
  # Install all binaries for all platforms  
  go run . tools dep local install --cross-platform
  ```
