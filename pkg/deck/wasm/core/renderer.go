// Package wasm provides functionality to render deck presentations from decksh DSL to various formats
package core
...
func NewDefaultRenderer() *Renderer {
	return NewRenderer(792.0, 612.0) // Letter size in points
}
...
func DefaultRenderOptions() RenderOptions {
	return RenderOptions{
		GridPercent: 0,
		Title:       "",
		Layers:      "image:rect:ellipse:curve:arc:line:poly:text:list",
	}
}
...
func (r *Renderer) XMLToSVG(xmlInput string, opts RenderOptions) (string, error) {
	...
	if len(d.Slide) > 0 {
		r.renderSlideToSVG(svgDoc, d, 0, opts)
	}
...
// This is adapted from the ajstarks/deck svgdeck implementation
func (r *Renderer) renderSlideToSVG(doc *svg.SVG, d deck.Deck, slideIndex int, opts RenderOptions) {
	...
	for _, layer := range layerList {
		...
		case "text":
			r.renderTexts(doc, slide.Text, slide.Fg, cw, ch)
		case "list":
			r.renderLists(doc, slide.List, slide.Fg, cw, ch)
		case "image":
...
func (r *Renderer) renderTexts(doc *svg.SVG, texts []deck.Text, defaultColor string, cw, ch float64) {
...
func (r *Renderer) renderLists(doc *svg.SVG, lists []deck.List, defaultColor string, cw, ch float64) {
