...
func TestDeckshToXMLDebug(t *testing.T) {
	r := NewDefaultRenderer()

	// Content from test/example.dsh
	dshInput := `deck 10 5

text "Hello Deck World!" 5 2.5 2
circle 5 2.5 1 "red" 0.5
rect 2 1 8 4 "gray" 0.3
text "My First Deck" 5 4.5 1.5
...
func TestDeckshToSVGWithRealExample(t *testing.T) {
	r := NewDefaultRenderer()
	opts := DefaultRenderOptions()
	opts.Title = "Example Deck"

	// Proper decksh syntax based on the linter
	dshInput := `deck
slide "white" "black"
...
edeck`
	...
	if err != nil {
		t.Fatalf("DeckshToSVG with real example failed: %v", err)
	}
