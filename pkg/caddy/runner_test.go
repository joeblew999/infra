...
func TestPresetConfigurations(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "Development preset includes bento playground",
			preset:         PresetDevelopment,
			port:          8080,
			expectedRoutes: 1,
			shouldHaveRoute: "/bento-playground/*",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.shouldHaveRoute != "" {
				found := false
				for _, route := range config.Routes {
					if route.Path == tt.shouldHaveRoute {
						found = true
						break
					}
				}
				if !found {
					t.Errorf("Expected route %s not found", tt.shouldHaveRoute)
				}
			}
		})
	}
