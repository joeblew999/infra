...
func TestNATSClusterConfiguration(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		if client <= 0 || cluster <= 0 || http <= 0 {
			t.Errorf("Invalid ports for node %d: client=%d, cluster=%d, http=%d", 
				i, client, cluster, http)
		}
		
		t.Logf("✅ Node %d ports - Client: %d, Cluster: %d, HTTP: %d", 
			i, client, cluster, http)
	}
